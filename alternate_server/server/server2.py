...
def list_files(path, file_array, dir_array):
	pass
	temp = glob.glob(path+"*")	
				# lists all the files and folderss in the path folder.
	print "\n\n\n\t\t ", path, "\n\n\n"			
	print temp, path
	#sleep(2)			
	for i in temp:
...
def sync_ker_server(client_socket,Path,array):
	...
	global LOGFILE_ADDR
	LOGFILE_ADDR = ".." + os.path.sep + 'dummy' + ".txt"
...
def main():
	...
	if(not Id):
		print "Hello World! "
		commands.getoutput("python ../server.py")		
	else:
		while 1:
			...
			if(not pid):
				...
				if ( (data[:5] == "Name:") and (os.path.isdir(os.path.sep+data[5:])) ):
					USERNAME = data[5:]
					USERDIR_ADDR = "" 			# in which data is to be saved.
				...
				while (1):
					...
					Path = USERDIR_ADDR 							# This depends on the user that has logged in now.
